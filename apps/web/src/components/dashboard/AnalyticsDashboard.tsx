  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="vfy-panel" style={{ padding: 24 }}>
          <h3 className="vfy-panel-title" style={{ marginBottom: 16 }}>
            Volume · last {data?.windowDays ?? 14} days
          </h3>
          <div className="h-[280px]">
            {byDay.length === 0 ? (
              <p className="vfy-settings-empty">No recent sessions to chart.</p>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={byDay}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--d-border)" vertical={false} />
                  <XAxis dataKey="day" fontSize={12} stroke="var(--d-muted)" />
                  <YAxis allowDecimals={false} fontSize={12} stroke="var(--d-muted)" />
                  <Tooltip />
                  <Bar dataKey="conversations" fill="var(--d-accent)" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
        ...
        <div className="vfy-panel" style={{ padding: 24 }}>
          <h3 className="vfy-panel-title" style={{ marginBottom: 16 }}>By channel</h3>
          <div className="h-[280px]">
            {byChannel.length === 0 ? (
              <p className="vfy-settings-empty">Channel breakdown appears after the first call.</p>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={byChannel} layout="vertical" margin={{ left: 24 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--d-border)" horizontal={false} />
                  <XAxis type="number" allowDecimals={false} fontSize={12} />
                  <YAxis type="category" dataKey="name" width={80} fontSize={12} />
                  <Tooltip />
                  <Bar dataKey="value" fill="var(--d-accent)" radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
      ...
      <div className="vfy-panel" style={{ overflow: 'hidden' }}>
        <div className="vfy-panel-head">
          <h3 className="vfy-panel-title">Recent conversations</h3>
        </div>
            ...
            <tbody>
              {(data?.recent ?? []).map((row) => (
                <tr key={row.id}>
                  ...
                  <td>{row.channel}</td>
                  <td><span className="vfy-tag">{row.status}</span></td>
                  <td>
              ...
              {!loading && (data?.recent.length ?? 0) === 0 && (
                <tr>
                  <td colSpan={5} className="vfy-settings-empty">
