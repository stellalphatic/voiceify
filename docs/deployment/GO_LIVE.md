...
**Signup note:** signup is self-serve — new users get a session immediately and land in the dashboard right after registering. There is no admin approval step. Admins can still suspend or reject any account afterwards in `/admin → Users`, which takes effect on the next sign-in.
