...
export function normalizeLanguageCode(raw?: string | null): LanguageCode {
  ...
  if (!token) return 'en';
  const fromIso3 = ISO3_TO_ISO1[token];
  if (fromIso3) return fromIso3;
  if (token.length === 2 && LANGUAGE_LABELS[token]) return token as LanguageCode;
...
function detectMarkedLatinLanguage(text: string): LanguageCode | null {
  ...
  const onlyLeader = leaders.length === 1 ? leaders[0] : undefined;
  if (onlyLeader) return onlyLeader.code;
  return 'en';
}
